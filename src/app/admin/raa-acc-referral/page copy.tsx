...
import { Trash2, Pencil, Save, Check, X, Plus, Upload, FileText, Download, Eye, Maximize2 } from "lucide-react";
...
export default function RaaAccReferralPage() {
  ...
  const [selectedView, setSelectedView] = useState<{ type: 'audit' | 'acc', data: string } | null>(null);
  ...
  return (
    <div className="flex">
      <Sidebar />
      <main className="flex-1 p-6 ml-64 bg-gray-100 min-h-screen overflow-x-auto">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th onClick={() => handleSort("accountability_entity")} className="px-4 py-3 text-left text-sm font-medium uppercase cursor-pointer select-none hover:bg-gray-100">
                    Entity {getSortIcon("accountability_entity")}
                  </th>
                ...
                {paginatedReferrals.length > 0 ? (
                  paginatedReferrals.map((ref, index) => (
                    <tr key={ref._id} className="hover:bg-gray-100 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm">{ref.ain}</td>
                      <td className="px-4 py-3 text-sm">{ref.para_no}</td>
                      <td className="px-4 py-3 text-sm max-w-[150px] truncate" title={ref.accountability_entity}>
                        {ref.accountability_entity}
                      </td>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[80vh] flex flex-col">
            <div className="flex justify-between items-center p-6 border-b">
              <h3 className="text-xl font-semibold">
                {selectedView.type === 'audit' ? 'Audit Report' : 'ACC Observation'}
              </h3>
              <button

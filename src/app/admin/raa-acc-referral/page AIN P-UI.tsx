...
export default function RaaAccReferralPage() {
  ...
  const handleAddOrUpdate = async () => {
    ...
    try {
      ...
      // Send existing attachments (pre-filled from duplicate AIN)
                ...
                <div className="mt-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {totalAttachmentsCount > 0 && (
                      <span className="text-xs text-gray-500">
                        Total: {totalAttachmentsCount} file(s)
                      </span>
                    )}
                  </div>
                    ...
                    <div className="space-y-2 mt-3">
                      ...
                      {!editData && showDuplicateWarning && (
                        <p className="text-xs text-green-600 mt-1">
                          ✓ These {existingAttachments.length} file(s) will be copied from the existing record
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {attachments.length > 0 && (
                    <div className="space-y-2 mt-3">
                      ...
                      <p className="text-xs text-green-600 mt-1">
                        ✓ {attachments.length} new file(s) will be added
                      </p>
                    </div>
                  )}

                  {/* Summary of attachments to be saved */}
                  {totalAttachmentsCount > 0 && (
                    <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                      <p className="text-sm font-medium text-gray-700">Summary:</p>
                      <ul className="text-xs text-gray-600 mt-1 space-y-1">
                        {existingAttachments.length > 0 && (
                          <li>• {existingAttachments.length} existing file(s) from original record</li>
                        )}
                        {attachments.length > 0 && (
                          <li>• {attachments.length} new file(s) being added</li>
                        )}
                        <li className="text-green-600 font-medium mt-1">
                          Total: {totalAttachmentsCount} file(s) will be saved with this referral
                        </li>
                      </ul>
                    </div>
                  )}

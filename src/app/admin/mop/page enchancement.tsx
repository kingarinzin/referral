...
type MOPRecord = {
  ...
  gewogName: string;
  villageName: string;
  photoBase64: string | null; // Changed from photoUrl
  reportedBy: string;
};
...
export default function MOPPage() {
  ...
  const [photoBase64, setPhotoBase64] = useState<string | null>(null); // NEW
    ...
    try {
      ...
      try {
        ...
        if (photoRes.ok) {
          ...
          // 🔥 NEW: Convert blob to base64 for storage
          ...
          // Keep blob URL for immediate display
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      const payload = {
        ...
        photoBase64: photoBase64, // 🔥 Send base64
        remarks: remarks,
  ...
  const handleReportMissing = async () => {
    ...
    setReporting(true);
    try {
      // First save as draft with photo (if not already saved)
      // Then report
      const payload = {
        cid: person.cid,
        name: person.fullName || `${person.firstName} ${person.lastName}`.trim(),
      ...
      // Save first
      const saveRes = await fetch('/api/mop/save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-4 py-2 text-left text-xs font-medium uppercase">Photo</th> {/* NEW */}

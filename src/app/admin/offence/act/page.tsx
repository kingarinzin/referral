...
export default function ActsPage() {
  ...
  const handleAdd = async () => {
    if (!formData.name.trim()) {
      showNotification("Act Name is required", "error");
      return;
    }
    try {
      const res = await fetch("/api/offences/act", {
      ...
      setShowForm(false);
      showNotification("Act added successfully", "success");
    } catch (error) {
      showNotification(
        error instanceof Error ? error.message : "Failed to add act",
        "error"
      );
    }
  };
  ...
  const handleEdit = (act: Act) => {
    setEditData(act);
    setFormData({ name: act.name, remarks: act.remarks });
    setShowForm(true);
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setEditData(null);
      setShowForm(false);
      showNotification("Act updated successfully", "success");
    } catch (error) {
      showNotification(
        error instanceof Error ? error.message : "Failed to update act",
        "error"
      );
    }
  };
  ...
  const handleDelete = async (act: Act) => {
    ...
    try {
      ...
      await fetchActs();
      showNotification("Act deleted successfully", "success");
    } catch (error) {
      showNotification(
        error instanceof Error ? error.message : "Failed to delete act",
        "error"
      );
    }
  };
  ...
  const handleSort = (key: keyof Act) => {
  ...
  const filteredActs = acts.filter(
    (act) =>
      ...
      (act.remarks?.toLowerCase() || "").includes(search.toLowerCase())
  );

  const sortedActs = [...filteredActs];
  if (sortConfig.key) {
    sortedActs.sort((a, b) => {
  ...
  const totalPages = Math.ceil(sortedActs.length / rowsPerPage);
  const startIndex = (currentPage - 1) * rowsPerPage;
  const paginatedActs = sortedActs.slice(
    startIndex,
    ...
    <div className="flex">
      ...
      <main className="flex-1 p-6 ml-64 bg-gray-100 min-h-screen">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Act List</h1>
          {!showForm && (
            <button
              onClick={() => {
                setShowForm(true);
        ...
        {showForm && (
          <div className="bg-white shadow rounded-xl p-6 mb-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium">Act Name</label>
                <input
                  name="name"
                  type="text"
                  value={formData.name}
              ...
              {paginatedActs.length > 0 ? (
                paginatedActs.map((act, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3 text-sm">{act.name}</td>
                    <td className="px-6 py-3 text-sm">{act.remarks}</td>

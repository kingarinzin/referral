...
export default function DepartmentsPage() {
  ...
  const [formData, setFormData] = useState({ name: "", remarks: "", agencyId: "" });
  ...
  // Fetch departments (now returns agencyName as well)
  ...
  const handleAdd = async () => {
    if (!formData.name || !formData.agencyId) {
      showNotification("Department Name and Agency are required", "error");
      return;
    }
    try {
      const res = await fetch("/api/departments", {
      ...
      setFormData({ name: "", remarks: "", agencyId: "" });
      setShowForm(false);
  ...
  const filteredDepartments = departments.filter(
    (dept) =>
      ...
      (dept.agencyName?.toLowerCase() || "").includes(search.toLowerCase()) ||
  ...
  const paginatedDepartments = sortedDepartments.slice(startIndex, startIndex + rowsPerPage);
      ...
      <main className="flex-1 p-6 ml-64 bg-gray-100 min-h-screen">
        ...
        <div className="flex justify-between items-center mb-4">
          <input
            ...
            placeholder="Search by name, agency or remarks..."
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                >
                  Agency {sortConfig.key === "agencyName" ? (sortConfig.direction === "asc" ? "▲" : "▼") : "▲▼"}
                </th>
                <th
              ...
              {paginatedDepartments.length > 0 ? (
                paginatedDepartments.map((dept, index) => (
                  <tr key={dept._id} className="hover:bg-gray-100 transition-colors">
                    ...
                    <td className="px-6 py-3 text-sm">{dept.agencyName || "—"}</td>
